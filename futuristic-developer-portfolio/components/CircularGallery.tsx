...
class Media {
  ...
  loadFallback(texture: any) {
    const fallback = new Image();
    fallback.crossOrigin = 'anonymous';
    fallback.src = `https://picsum.photos/seed/${this.text}/800/600?grayscale`;
    fallback.onload = () => {
      texture.image = fallback;
      if (this.program?.uniforms?.uImageSizes) {
        this.program.uniforms.uImageSizes.value = [fallback.naturalWidth, fallback.naturalHeight];
      }
    };
  }

  isTainted(img: HTMLImageElement): boolean {
    try {
      const canvas = document.createElement('canvas');
      canvas.width = 1;
      canvas.height = 1;
  ...
  createShader() {
    ...
    const img = new Image();
    if (!this.image.startsWith('blob:') && !this.image.startsWith('data:')) {
      img.crossOrigin = 'anonymous';
    }
    img.src = this.image;
    img.onload = () => {
      if (this.isTainted(img)) {
        this.loadFallback(texture);
        return;
      }
      try {
        texture.image = img;
...
class GalleryApp {
  ...
  onTouchUp(e: any) {
    this.isDown = false;
    const endX = e.changedTouches ? e.changedTouches[0].clientX : (e.clientX || this.start);
    const endY = e.changedTouches ? e.changedTouches[0].clientY : (e.clientY || this.startY);
  ...
  handleClick(x: number, y: number) {
    ...
    if (closestMedia && minDistance < closestMedia.plane.scale.x / 1.5 && this.onItemClick) {
        this.onItemClick(this.mediasImages[closestMedia.index]);
    }
  }
  onWheel(e: any) {
