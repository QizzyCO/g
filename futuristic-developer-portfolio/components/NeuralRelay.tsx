...
import { Send, User, Shield, Terminal, Radio, Cpu, Clock, Edit2, Check, X as CloseIcon, Globe, RefreshCw } from 'lucide-react';
...
// Using a dedicated public store URL. If it fails, the system auto-bootstraps.
const GRID_STORE_URL = 'https://api.npoint.io/c51d650702d184766863'; 
...
const NeuralRelay: React.FC = () => {
  ...
  const fetchGlobalFeed = async () => {
    ...
    try {
      ...
      // If the bin is not found (404), it's just uninitialized. Treat as empty.
      ...
      if (!response.ok) throw new Error('Network protocol mismatch');
      ...
      console.warn('Relay Sync Warning:', err);
      setSyncStatus('error');
  ...
  useEffect(() => {
    ...
    pollTimerRef.current = window.setInterval(fetchGlobalFeed, 30000); // Poll every 30s to keep sync
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error('Grid unreachable');
      }
      ...
      const updated = [...latestMessages, msg].slice(-50); // Keep last 50 for performance
    ...
    <div className="w-full max-w-5xl mx-auto px-4">
      <div className="text-center mb-12">
        <span className="text-blue-500 mono text-xs tracking-[0.5em] mb-2 uppercase font-bold flex items-center justify-center gap-2">
          <Radio size={12} className={syncStatus === 'syncing' ? 'animate-spin' : 'animate-pulse'} /> 
          {syncStatus === 'syncing' ? 'Syncing with Grid...' : 'Global Relay Feed'}
        </span>
        ...
        <div className="mt-6 flex flex-col items-center">
          ...
          <div className="flex items-center gap-4 mt-2">
            <p className="text-gray-500 mono text-[8px] uppercase tracking-[0.2em] flex items-center gap-2">
              <Globe size={10} className={syncStatus === 'error' ? 'text-red-500' : 'text-blue-500'} />
              Status: {syncStatus === 'error' ? 'UPLINK DISRUPTED' : 'GRID CONNECTED'}
            </p>
            {syncStatus === 'error' && (
              <button 
      ...
      <div className="relative bg-[#0d162d]/40 border border-blue-500/20 rounded-[2.5rem] overflow-hidden backdrop-blur-xl shadow-2xl">
          ...
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-2">
               ...
               <span className={`mono text-[10px] ${syncStatus === 'error' ? 'text-red-500' : 'text-green-500'} uppercase tracking-widest`}>
                 {syncStatus === 'error' ? 'Offline Mode' : 'Public Uplink'}
               </span>
             </div>
             <div className="h-4 w-[1px] bg-white/10 hidden md:block" />
             <div className="hidden md:flex items-center gap-2 text-blue-400/60">
               <Cpu size={12} />
               <span className="mono text-[8px] uppercase">Node Sync: {messages.length} Packets</span>
             </div>
          </div>
          <div className="flex items-center gap-3">
             <span className="mono text-[8px] text-blue-500 uppercase hidden sm:inline">RELAY_PROTOCOL: V2</span>
          </div>
        </div>
          ...
          <AnimatePresence initial={false}>
            {messages.length === 0 && syncStatus !== 'syncing' ? (
               <div className="flex-1 flex items-center justify-center opacity-20">
                  <p className="mono text-xs uppercase tracking-widest text-center">Protocol initialized. <br/>Waiting for broadcast...</p>
               </div>
            ) : messages.map((msg) => (
              <motion.div
                key={msg.id}
        ...
        <div className="p-6 border-t border-white/5 bg-[#0a050d]/40 relative z-20">
          <form onSubmit={handleSubmit} className="flex gap-4">
            <div className="flex-1 relative">
              <input 
                ...
                placeholder="Broadcast to global grid..."
            ...
            <button 
              ...
              className={`px-8 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-xl active:scale-95 ${
                ...
                : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-500/20 cursor-target'
              }`}
            >
              {isSubmitting ? (
          ...
          <div className="mt-4 flex items-center justify-between text-[8px] mono text-gray-600 uppercase tracking-widest">
            <span className="flex items-center gap-1"><Clock size={10} /> 30-Day Auto-Purge active</span>
            <span className="text-blue-500/40 font-bold">DEVICE SYNC: ACTIVE</span>
          </div>
        </div>
      </div>
    </div>
