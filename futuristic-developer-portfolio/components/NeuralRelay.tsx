...
import { Send, User, Shield, Terminal, Radio, Cpu, Clock, Edit2, Check, X as CloseIcon, Globe, RefreshCw, Zap, Wifi } from 'lucide-react';
...
const NeuralRelay: React.FC = () => {
  ...
  const fetchGlobalFeed = async (showLoading = true) => {
    ...
    try {
      // CACHE BUSTING: Appending timestamp forces the browser to fetch NEW data from the server
      const cacheBuster = `?t=${Date.now()}`;
      const response = await fetch(`${GRID_STORE_URL}${cacheBuster}`);
      ...
      // Filter out messages older than 30 days and ensure unique IDs
  ...
  useEffect(() => {
    ...
    // POLLED SYNC: Check for new messages from other devices every 5 seconds
    pollTimerRef.current = window.setInterval(() => fetchGlobalFeed(false), 5000);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. FRESH FETCH: Get latest state immediately before posting (with cache-bust)
      const fetchResponse = await fetch(`${GRID_STORE_URL}?cb=${Date.now()}`);
      let latestMessages = [];
      if (fetchResponse.ok) {
        const currentGrid = await fetchResponse.json();
    ...
    <div className="w-full max-w-5xl mx-auto px-4">
      <div className="text-center mb-12">
        <span className="text-blue-500 mono text-xs tracking-[0.5em] mb-2 uppercase font-bold flex items-center justify-center gap-2">
          ...
          {syncStatus === 'syncing' ? 'Transmitting...' : 'Shared Neural Link'}
        </span>
        ...
        <div className="mt-6 flex flex-col items-center">
          ...
          <div className="flex items-center gap-4 mt-3">
            <p className="text-gray-500 mono text-[8px] uppercase tracking-[0.2em] flex items-center gap-2">
              <Globe size={10} className={syncStatus === 'error' ? 'text-red-500' : 'text-green-500'} />
              Node Status: {syncStatus === 'error' ? 'SYNC ERROR' : 'GRID_STABLE'}
            </p>
            {syncStatus === 'error' && (
              <button 
      ...
      <div className="relative bg-[#0d162d]/40 border border-blue-500/20 rounded-[2.5rem] overflow-hidden backdrop-blur-xl shadow-2xl">
          ...
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-2">
               ...
               <span className={`mono text-[10px] ${syncStatus === 'error' ? 'text-red-500' : 'text-green-500'} uppercase tracking-widest`}>
                 {syncStatus === 'error' ? 'Local Buffer' : 'Global Relay'}
               </span>
             </div>
             <div className="h-4 w-[1px] bg-white/10 hidden md:block" />
             <div className="hidden md:flex items-center gap-2 text-blue-400/60">
               <Wifi size={12} />
               <span className="mono text-[8px] uppercase">Strength: {Math.round(signalStrength)}%</span>
             </div>
          </div>
          <div className="flex items-center gap-3">
             <span className="mono text-[8px] text-blue-500 uppercase">Sync: {syncStatus === 'syncing' ? 'BUSY' : 'IDLE'}</span>
             <div className="w-16 h-1 bg-white/5 rounded-full overflow-hidden">
                <motion.div 
                  initial={{ x: '-100%' }}
                  animate={syncStatus === 'syncing' ? { x: '100%' } : { x: '0%' }}
          ...
          <AnimatePresence initial={false}>
            {messages.length <= 1 && syncStatus === 'idle' ? (
               <div className="flex-1 flex flex-col items-center justify-center opacity-20 text-center gap-4">
                  ...
                  <p className="mono text-xs uppercase tracking-widest">Scanning Grid for signals...<br/>Broadcast a message to begin.</p>
               </div>
            ) : messages.map((msg) => (
              <motion.div
                key={msg.id}
                initial={{ opacity: 0, x: msg.sender === nodeId ? 20 : -20 }}
        ...
        <div className="p-6 border-t border-white/5 bg-[#0a050d]/40 relative z-20">
          <form onSubmit={handleSubmit} className="flex gap-4">
            <div className="flex-1 relative">
              <input 
                ...
                placeholder="Broadcast a message to all devices..."
          ...
          <div className="mt-4 flex items-center justify-between text-[8px] mono text-gray-600 uppercase tracking-widest">
            <span className="flex items-center gap-1"><Cpu size={10} /> Encryption: AES-256 Enabled</span>
            <span className="text-blue-500/40 font-bold uppercase">Cross-Device Sync: Global</span>
          </div>
        </div>
      </div>
    </div>
